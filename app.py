...
# Auto-detects from .streamlit/secrets.toml or Streamlit Cloud Deployment Secrets
...
def handle_submit():
    """Process user input via a stable Streamlit callback mechanism."""
    user_text = st.session_state.user_input_text.strip()
    if user_text:
        if not st.session_state.api_key:
            st.session_state.api_error_msg = "⚠️ Please configuration your GROQ_API_KEY in Streamlit secrets."
            return

        st.session_state.api_error_msg = None
        ...
        # Clear input text field via its session state key
        st.session_state.user_input_text = ""

        # Request Groq call flag
        st.session_state.trigger_ai_call = True


# Initialize missing state controls
if "api_error_msg"    not in st.session_state: st.session_state.api_error_msg = None
if "trigger_ai_call"  not in st.session_state: st.session_state.trigger_ai_call = False
...
with st.sidebar:
    ...
    st.markdown('<div class="mode-label">Security Settings</div>', unsafe_allow_html=True)
    if st.session_state.api_key:
        st.success("🔒 API Key loaded from Secrets")
    else:
        st.warning("🔑 Keys missing in secrets.toml")
        # Fallback input if secrets are missing
        fallback_key = st.text_input("Temporary Groq Key", type="password", placeholder="gsk_...")
        if fallback_key:
...
if st.session_state.api_error_msg:
    st.error(st.session_state.api_error_msg)

# ── Handle AI Response Execution ──────────────────────────────────────────────
if st.session_state.trigger_ai_call:
    st.session_state.trigger_ai_call = False  # Reset flag immediately
    ...
    with st.spinner("Nexo AI is thinking..."):
        try:
            ...
            st.session_state.msg_count += 1
            st.rerun()  # Reload once to draw the new message stably

        except requests.exceptions.HTTPError as e:
            status = e.response.status_code if e.response else "?"
            if status == 401:
                st.session_state.api_error_msg = "❌ Invalid API key. Please check your Groq API key configuration."
            elif status == 429:
                st.session_state.api_error_msg = "⚠️ Rate limit hit. Please wait a moment and try again."
            else:
                st.session_state.api_error_msg = f"❌ API error {status}: {e}"
            st.rerun()
        except Exception as e:
            st.session_state.api_error_msg = f"❌ Error: {str(e)}"
            st.rerun()
...
# Render submission layout using custom session state and callback
